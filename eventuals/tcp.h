...
class AcceptedSocket {
 public:
  ...
  AcceptedSocket& operator=(AcceptedSocket&& that) = delete;
  ...
  auto Read(const size_t& bytes_to_read) {
    ...
    return RescheduleAfter(
        ...
        _Read::Composable{loop_, tcp_, bytes_to_read});
  }
  ...
  auto Write(const std::string& data) {
    ...
    return RescheduleAfter(
        // TODO(benh): borrow '&loop' so accept can't outlive a loop.
        _Write::Composable{loop_, tcp_, data});
  }
  ...
  auto Shutdown() {
    ...
    return RescheduleAfter(
        // TODO(benh): borrow '&loop' so accept can't outlive a loop.
        _Shutdown::Composable{loop_, tcp_});
  }
 ...
 private:
            ...
            [this]() {
              if (!completed_) {
                ...
                int status = uv_read_start(
                    stream(),
                    [](uv_handle_t* handle, size_t suggested_size, uv_buf_t* buffer) {
                    ...
                    [](uv_stream_t* stream, ssize_t nread, const uv_buf_t* buffer) {
                      ...
                      if (nread < 0) {
                        ...
                        if ((size_t) nread < continuation.bytes_to_read_) {
                          ...
                          continuation.buffer_ += std::string(buffer->base, continuation.bytes_to_read_);
  ...
  struct _Write {
    ...
    struct Continuation {
      ...
      // Already started uv_write can't be interrupted.
      // http://docs.libuv.org/en/v1.x/request.html?highlight=uv_req_t#c.uv_cancel
      void Register(class Interrupt& interrupt) {
        k_.Register(interrupt);
  ...
  struct _Shutdown {
    ...
    struct Continuation {
      ...
      // Already started uv_shutdown can't be interrupted.
      // http://docs.libuv.org/en/v1.x/request.html?highlight=uv_req_t#c.uv_cancel
      void Register(class Interrupt& interrupt) {
        k_.Register(interrupt);
  ...
  struct _Close {
    ...
    struct Continuation {
      ...
      // Already started uv_close can't be interrupted.
      // http://docs.libuv.org/en/v1.x/request.html?highlight=uv_req_t#c.uv_cancel
      void Register(class Interrupt& interrupt) {
        k_.Register(interrupt);
...
struct _Accept {
  ...
  struct Continuation : public detail::TypeErasedStream {
    Continuation(K_ k, EventLoop& loop, std::string&& ip, const int port)
      : k_{std::move(k)},
        ...
        next_(&loop, "Accept (next)"),
        done_(&loop, "Accept (done)") {}
    ...
    Continuation(Continuation&& that)
      : k_{std::move(that.k_)},
        ...
        next_(&that.loop_, "Accept (next)"),
        done_(&that.loop_, "Accept (done)") {
      CHECK(!that.started_ || !that.completed_) << "moving after starting";
      CHECK(!handler_);
    }
    ...
    void Start() {
      ...
      // Saving the current scheduler context to be used later.
      k_();
   ...
   private:
    ...
    Reschedulable<K_, AcceptedSocket> k_;
    //K_ k_;
    EventLoop& loop_;
    std::string ip_;
    int port_;
    ...
    EventLoop::Waiter start_;
    EventLoop::Waiter interrupt_;
    EventLoop::Waiter next_;
    EventLoop::Waiter done_;
  ...
  struct Composable {
    template <typename>
    using ValueFrom = uv_tcp_t*;

    template <typename Arg, typename K>
    auto k(K k) && {
      return Continuation<K, Arg>{std::move(k), loop_, std::move(ip_), port_};
    }
...
inline auto Accept(
  ...
  //return RescheduleAfter(
  //    // TODO(benh): borrow '&loop' so accept can't outlive a loop.
  //    _Accept::Composable{loop, ip, port});

  return _Accept::Composable{loop, ip, port};
}

inline auto Accept(
    const std::string& ip = "0.0.0.0",
    const int port = 0) {
  return Accept(EventLoop::Default(), ip, port);
}
...
//                    return Read() | Timer() | Write() | Close(client_socket);
//                 });